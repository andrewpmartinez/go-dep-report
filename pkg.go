...
func (p *Pkg) ParentPkgName() string {
...
func (p *Pkg) LicenseName() string {
	if p == nil || p.Raw == nil || p.Raw.Dir == "" {
		return "???"
	}
	licenseFile := filepath.Join(p.Raw.Root, "LICENSE")
	...
	header := strings.Join(strings.Split(string(content), "\n")[:2], " ")

	re := regexp.MustCompile(`\s+`)
	strippedHeader := re.ReplaceAllString(header, " ")
	return strippedHeader
}

// Resolve recursively finds all dependencies for the Pkg and the packages it depends on.
func (p *Pkg) Resolve(ctx Context, i Importer) {
	...
	p.setDeps(ctx, i, pkg.Imports, pkg.Dir, unique, false)
	if p.Tree.ResolveTest {
		p.setDeps(ctx, i, append(pkg.TestImports, pkg.XTestImports...), pkg.Dir, unique, true)
	}
}
...
// to the Pkg.
func (p *Pkg) setDeps(ctx Context, i Importer, imports []string, srcDir string, unique map[string]struct{}, isTest bool) {
	for _, imp := range imports {
		...
		p.addDep(ctx, i, imp, srcDir, isTest)
	}
...
func (p *Pkg) addDep(ctx Context, i Importer, name string, srcDir string, isTest bool) {
	dep := Pkg{
		Name:   name,
		SrcDir: srcDir,
		Tree:   p.Tree,
