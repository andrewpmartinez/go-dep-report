...
func ErrRootPkgNotResolved(pkg string, err error) error {
	return fmt.Errorf("unabled to resolve root package %s: %w", pkg, err)
}
...
func (t *Tree) Resolve(ctx Context, name string) error {
	pwd, err := os.Getwd()
	if err != nil {
		return err
	}
	...
	t.Root.Resolve(ctx, t.Importer)
	if !t.Root.Resolved {
		return ErrRootPkgNotResolved(name, t.Root.Error)
	}
